...
then
	...
	$spark $rumble --show-error-info yes --shell yes	
else
	echo "Submitting Ruble query located at $1"
	$spark $rumble --show-error-info yes --query-path $1 # --output-path "hdfs:///user/me/results/output" --log-path "hdfs:///user/me/logging/mylog"
fi
