...
let $filtered := (
	...
	let $pairs := (
		for $iIdx in (1 to (size($leptons.pt) - 1))
			return for $jIdx in (($iIdx + 1) to size($leptons.pt))
				...
				return {"i": $iIdx, "j": $jIdx, "mass": abs(91.2 - hep:AddPtEtaPhiM2($particleOne, $particleTwo).mass)}
	)
	where exists($pairs)
	...
	let $otherLeptonMass := (
		...
		let $transverseMass := 2 * $i.MET_pt * $maxOtherPt * (1.0 - cos(hep:DeltaPhi($i.MET_phi, $leptons.phi[[$j]])))
